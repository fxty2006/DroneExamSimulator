...
> **ℹ️ アンインストールについて**
> 本アプリは **フォルダを削除するだけ** でアンインストール可能です。
> ただし、手順3で自動インストールされた **Python本体** はPCに残ります。アプリを削除してもPythonは消えませんので、不要な場合はWindowsの設定から別途削除してください。
...
### アプリの削除
レジストリを使用しないポータブル形式のため、**本アプリのフォルダごと削除するだけ** で完了です。

### Pythonの削除（必要な場合）
本アプリを起動するために **Python本体** を自動（または手動）でインストールした場合、アプリのフォルダを削除してもPython本体はPCに残ります。
今後Pythonを使用する予定がない場合は、以下の手順で別途削除してください。

1.  Windowsの「設定」→「アプリ」→「インストールされているアプリ」を開く。
2.  検索バーに `Python` と入力。
3.  `Python 3.12.x` (またはインストールしたバージョン) をアンインストールする。
